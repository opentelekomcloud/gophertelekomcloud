...
// Get is used to query the product specifications list.
func Get(client *golangsdk.ServiceClient, engine string) (*GetResp, error) {
	...
	// Here is a dirty hack for providing a right url
	// We should send a GET to /v2/{engine}/products

	paths := strings.SplitN(client.Endpoint, "v2", 2)
	url := fmt.Sprintf("%sv2/%s/%s", paths[0], engine, resourcePath)
