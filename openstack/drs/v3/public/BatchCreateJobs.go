...
type CreateJobOpts struct {
	...
	// Enterprise project. If this parameter is not specified, the value is default.
	// The key must be _sys_enterprise_project_id, and the value is the enterprise project ID.
	// Only one enterprise project can be selected.
...
type Endpoint struct {
	...
	// The database name can be a maximum of 128 characters in length and cannot contain the following
	// special characters: !<>&'\" The name of the backup database refers to the name of the database
	// contained in the backup file. You can select one or more databases to be restored as required.
	...
	// Values: 0: Normal cluster 1: Security cluster
