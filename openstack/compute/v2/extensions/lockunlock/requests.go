...
func Lock(client *golangsdk.ServiceClient, id string) (r LockResult) {
	raw, err := client.Post(actionURL(client, id), map[string]interface{}{"lock": nil}, nil, nil)
	return
}
...
func Unlock(client *golangsdk.ServiceClient, id string) (r UnlockResult) {
	raw, err := client.Post(actionURL(client, id), map[string]interface{}{"unlock": nil}, nil, nil)
	return
}
