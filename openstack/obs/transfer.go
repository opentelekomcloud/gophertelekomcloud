...
func getCheckpointFile(ufc *UploadCheckpoint, uploadFileStat os.FileInfo, input *UploadFileInput, obsClient *ObsClient) (needCheckpoint bool, err error) {
	...
	if err != nil {
		...
		if ufc.Bucket != "" && ufc.Key != "" && ufc.UploadId != "" {
			_err := abortTask(ufc.Bucket, ufc.Key, ufc.UploadId, obsClient)
			if _err != nil {
...
func prepareUpload(ufc *UploadCheckpoint, uploadFileStat os.FileInfo, input *UploadFileInput, obsClient *ObsClient) error {
...
func abortTask(bucket, key, uploadID string, obsClient *ObsClient) error {
...
func handleUploadFileResult(uploadPartError error, ufc *UploadCheckpoint, enableCheckpoint bool, obsClient *ObsClient) error {
...
func completeParts(ufc *UploadCheckpoint, enableCheckpoint bool, checkpointFilePath string, obsClient *ObsClient, encodingType string) (output *CompleteMultipartUploadOutput, err error) {
...
func (obsClient ObsClient) resumeUpload(input *UploadFileInput) (output *CompleteMultipartUploadOutput, err error) {
	...
	if enableCheckpoint {
		needCheckpoint, err = getCheckpointFile(ufc, uploadFileStat, input, &obsClient)
		if err != nil {
			return nil, err
		}
	}
	if needCheckpoint {
		err = prepareUpload(ufc, uploadFileStat, input, &obsClient)
		if err != nil {
			return nil, err
		}
	...
	uploadPartError := obsClient.uploadPartConcurrent(ufc, checkpointFilePath, input)
	err = handleUploadFileResult(uploadPartError, ufc, enableCheckpoint, &obsClient)
	if err != nil {
		return nil, err
	}

	completeOutput, err := completeParts(ufc, enableCheckpoint, checkpointFilePath, &obsClient, input.EncodingType)
...
func (obsClient ObsClient) uploadPartConcurrent(ufc *UploadCheckpoint, checkpointFilePath string, input *UploadFileInput) error {
...
func getObjectInfo(input *DownloadFileInput, obsClient *ObsClient) (getObjectmetaOutput *GetObjectMetadataOutput, err error) {
...
func (obsClient ObsClient) resumeDownload(input *DownloadFileInput) (output *GetObjectMetadataOutput, err error) {
	getObjectmetaOutput, err := getObjectInfo(input, &obsClient)
	if err != nil {
		return nil, err
	}
	...
	downloadFileError := obsClient.downloadFileConcurrent(input, dfc)
...
func (obsClient ObsClient) downloadFileConcurrent(input *DownloadFileInput, dfc *DownloadCheckpoint) error {
	pool := NewRoutinePool(input.TaskNum, MAX_PART_NUM)
